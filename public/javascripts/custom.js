...
$(function () {
    // The event listener for the file upload
    if (document.getElementById('multipleFileUpload') != null) {
        document.getElementById('multipleFileUpload').addEventListener('change', upload, false);
    }
        // Method that checks that the browser supports the HTML5 File API
        function browserSupportFileUpload() {
            var isCompatible = false;
        ...
        function upload(evt) {
            if (!browserSupportFileUpload()) {
                ...
                var data = null;
                initSceneTimeSeries(evt.target.files);
            }
        }
});
